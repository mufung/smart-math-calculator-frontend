// ============================================================
// app.js — Math AI Assistant — Path 11
// Now shows Wolfram Alpha badge + steps alongside SymPy badge
// ============================================================
...
function addSympyTrustBadge(messageWrapper, sympyAnswer, topic) {
    ...
    badge.innerHTML =
        ...
        "<div class='sympy-badge-body'>" +
            "<span class='sympy-badge-answer'>" +
                escapeHtml(sympyAnswer) +
            "</span>" +
            "<span class='sympy-badge-note'>" +
                "Computed by SymPy symbolic math engine — 100% exact" +
            "</span>" +
        "</div>";
...
// ── WOLFRAM ALPHA BADGE ───────────────────────────────────────
function addWolframBadge(messageWrapper, answer, steps,
                          altForms, crossVerified) {
    if (!messageWrapper) return;

    var stepsHtml = "";
    if (steps && steps.length > 0) {
        stepsHtml =
            "<div class='wolfram-steps' id='wolfram-steps-" +
            Date.now() + "'>" +
    ...
    var crossHtml = crossVerified
        ? "<div class='wolfram-cross-verify'>" +
          "🔗 Cross-verified with SymPy — doubly confirmed correct!" +
          "</div>"
        : "";
    ...
    badge.innerHTML =
        "<div class='wolfram-badge-header'>" +
            ...
            "<button class='wolfram-expand-btn' onclick='toggleWolframDetails(\"" + uid + "\")'>" +
                "Details ▼" +
            "</button>" +
        "</div>" +
        ...
        "<div class='wolfram-details hidden' id='" + uid + "'>" +
            stepsHtml +
            altHtml +
            "<div class='wolfram-source-note'>" +
                "Source: Wolfram Alpha — the world\'s most trusted computational engine" +
            "</div>" +
        "</div>";

    // Insert after SymPy badge if present
...
function addVerificationBadge(messageWrapper, result) {
    ...
    badge.innerHTML =
        ...
        "<span class='badge-label'>" +
            escapeHtml(result.badge_text || "Checked") +
        "</span>" +
        (result.computed_answer
            ? "<span class='badge-answer'>= " +
              escapeHtml(result.computed_answer) + "</span>"
            : "") +
        "<span class='badge-details-toggle' " +
            "onclick='toggleBadgeDetails(this)'>▼</span>" +
        "<div class='badge-details hidden'>" +
            escapeHtml(result.message || "") +
            (result.confidence
                ? "<span class='badge-confidence'>Confidence: " +
...
var allY       = points.map(function(p) { return p.y; });
    ...
    var xTickStep  = xRange2 <= 4 ? 0.5 : xRange2 <= 8 ? 1 :
                     xRange2 <= 16 ? 2 : 5;
